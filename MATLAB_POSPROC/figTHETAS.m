function figTHETAS(X1, Y1, cr)
%CREATEFIGURE(X1,Y1)
%  X1:  vector of x data
%  Y1:  vector of y data

%  Auto-generated by MATLAB on 28-Oct-2013 10:39:49
...
% Create axes
axes2 = axes('Parent',figure2,'FontSize',14,'FontName','Times New Roman',...
    'DataAspectRatio',[1 (B-A)/(D-C) 1]);

box(axes2,'on');
hold(axes2,'all');

% Create plot
plot1 = plot(X1,Y1,'MarkerEdgeColor',[0 0 0],'Marker','o','LineStyle','none');
xlim(axes2,[C D])
ylim(axes2,[A B])

% Create xlabel
xlabel('$\theta_\phi$','Interpreter','latex','FontSize',20,...
    'FontName','Times New Roman');

% Create ylabel
ylabel('$\theta_k$','Interpreter','latex','FontSize',20,...
    'FontName','Times New Roman');
% Correlation 
CR=['$R = ' num2str(cr(1,2),'%3.2f') '$']
text(.05,.85,CR,'parent',axes2, ...
    'verticalalignment','top','units','normalized','FontSize',16,...
    'FontName','Times New Roman','FontAngle','italic','Interpreter','latex');
...
% fitResults1 = robustfit(xdata1, ydata1)';
% Evaluate polynomial
...
% Plot the fit
fitLine1 = plot(xplot1,yplot1,'DisplayName','Adj.','Parent',axes2,...
    'Tag','linear','Color',[0 0 1],'LineWidth',2);

% Set new line in proper position
setLineOrder(axes2, fitLine1, plot1);

% "Show equations" was selected
showEquations(fittypesArray1, coeffs1, 2, axes2);
