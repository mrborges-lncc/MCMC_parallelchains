...
inp = load('../simuladorRigido/exp/input_cond.in');
%
if(TIPOINPUT2 == 1)
    name = '../SIMULADOR_VISCOELASTICO/expDOMO/input_cond.in'
...
%[FILENAME, PATHNAME] =uigetfile('../simuladorRigido/exp/fields/*.dat', 'LOAD DATA');
...
for II=ini:1:fim
    ...
    for j=ny:-1:1
        ...
        for i=1:nx
            k=k+1;
            permmap(j,i)=log(M*exp(S*mattamp(k)));
        end
        k=k+1;
    ...
    y = s(:,1);
    if(controle==0)
        ma=max(max(permmap))*1.0
        mi=min(min(permmap))*1.0
    end
    if(quad==1)
...
if(permfigs==1)
    figure1 = figure(...
      ...
      'PaperPosition',[0.6345 0.6345 11.7 2.7],...
      'PaperSize',[12 3],...
      'PaperType','<custom>');
