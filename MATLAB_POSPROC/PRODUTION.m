clear;
close all;
base_name = 'prod_D2_KLfull_DE_RK0_250';
%dados=load('../prod/prodF_ref_0.dat');
%dados=load('../SIMULADOR_ELASTICO/exp/conc/conc_ref_0.dat');
dados=load('~/MCMCde/twophaseflow/exp/prod/prod_ref_0.dat');
ref=dados;
%
file_name = ['~/MCMCde/twoStage/select_prod/' base_name '.dat']
%
% Create figure
figure1 = figure()
B=500;
...
plot(dados(:,1),dados(:,2),'Parent',axes1,'Color',[1 0 0],...
    'MarkerSize',4,'Marker','o','LineStyle','none','DisplayName','sample 1')
...
ylim(axes1,[A B])
% Create xlabel
xlabel('time ($day$)','FontSize',20,'FontName','Times New Roman',...
    'FontAngle','italic','Interpreter', 'latex');

% Create ylabel
ylabel('Oil rate ($m^3/day$)','FontSize',20,'FontName','Times New Roman',...
    'FontAngle','italic','Interpreter', 'latex');
% Create legend
...
norma=norm(ref(:,2:N)-dados(:,2:N))/norma;
