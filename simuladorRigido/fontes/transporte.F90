...
!
      ...
      private :: maxder, courant, ktdd_rt, fkt, nbflux, derivau, udd_rt, LOCAL_MASS_BAL
...
!     Scheme: KT
      ...
      use mGlobaisEscalares, only: ndofV, tTransporte,dtBlocoTransp,nnp
...
!     Kurganov-Tadmor DxD
!
      call ktdd_rt(nsd,nprt,ntime,nt,nen,numnp,numel, &
...
!        if(tid==0)print*, "em maxder, numThreads= ", omp_get_num_threads()
...
!=======================================================================
!     
      subroutine ktdd_rt(nsd,nprt,ntime,nt,nen,numnp,numel,&
...
!     
        ...
        use mLeituraEscrita,   only: PRINT_VTK, nprtpres,nprtsat
        use mLeituraEscrita,   only: isatTransiente,paraview_escalarPorElementoTransiente
        use mLeituraEscrita,   only: iflag_masl,iflag_sat,iflag_tipoPrint, isat,ifsat_out
        use mLeituraEscrita,   only: iflag_pres,iflag_tipoPrint, ipres,ifpres_out
        use mLeituraEscrita,   only: tprt_masl,dtprt_masl, tprt_sat,dtprt_sat,tprt_pres,dtprt_pres
...
!$OMP SINGLE
!
!      atualiza o tempo de simulacao
!
           tTransporte=tTransporte+dt
!     
!.... calcula e imprime o balanco de massa do sistema
! !
!          if(tTransporte.gt.tprt_masl) then
!            if(iflag_masl==1) call LOCAL_MASS_BAL(nsd,numel, &
!              nen,phi,v,tTransporte,permkx,permky,permkz, &
!              conecNodaisElem,listaDosElemsPorNo,dt,uf,du, inicioSol, fimSol)
!          tprt_masl=tprt_masl+dtprt_masl
!          end if 
!$OMP END SINGLE
...
!
           do nel=inicioSol,fimSol
              ...
              if(satElem(nel).gt.umaxAux(tid))then
                 umaxAux(tid) = satElem(nel)
                 nelSmax      = nel
              endif
              if(satElem(nel).lt.uminAux(tid)) then
                 uminAux(tid) = satElem(nel)
                 nelSmin      = nel
              endif
         !write(*,*) nel, satElem (nel)
           end do
!$OMP BARRIER
!$OMP MASTER


!       if(strang.eqv..false.) call satcor(nsd,numelReserv,mat,c,pressaoElem,pressaoElemAnt,phi,phi0,satElem,satElemAnt,dt) 
!       if(strang.eqv..false.) CALL SATCOR1(numelReserv,phi,phi0,satElem,satElemAnt,dt) 

!      write(*,'(a,i0,2x,a,3f12.5)') "passo", ns,'massa global = ',sum(satElem), maxval(satElem), minval(satElem)
         !
!
           IF(NIFLAG_CONC.EQ.1)THEN
              IF(TPRT_CONCM.LE.tTransporte)THEN
                 call MONITOR(filecout,pcondc,npcondc,elem_condc,satElem,tTransporte)
                 TPRT_CONCM = TPRT_CONCM+DTPRT_CONCM
              END IF
           END IF
           IF(NIFLAG_PRES.EQ.1)THEN
              IF(TPRT_PRESM.LE.tTransporte)THEN
                 call MONITOR(filepout,pcondp,npcondp,elem_condp,pressaoElem,tTransporte)
                 TPRT_PRESM = TPRT_PRESM+DTPRT_PRESM
              END IF
           END IF
           IF(NIFLAG_PROD.EQ.1)THEN
              IF(TPRT_PRODM.LE.tTransporte)THEN
                 call MONITORPROD(fileprout,NPCONDPR,v,satElem,permkx,permky,permkz,tTransporte)      
                 TPRT_PRODM = TPRT_PRODM+DTPRT_PRODM
              END IF
           END IF
...
!     
         if(tTransporte.gt.tprt_sat) then
            if(iflag_sat==1) then 
...
!     
         if(tTransporte.gt.tprt_pres) then
...
! 
!       write(*,*) " "
!        write(*,*) "  Smax:", maxval(umaxAux),"  Smin:", minval(uminAux)
!       write(*,*) " "
       write(*,"(//,'Smax: ',f15.7,2x,'Smin: ',f15.7,//)") maxval(umaxAux), minval(uminAux)

!  
     end subroutine ktdd_rt
!
!=======================================================================
!     
...
!     
      ...
      contains
         ...
         if(nsd==3)n(5)= listaDosElemsPorFace(6,conecLadaisElem(5,nel))
         if(nsd==3)n(6)= listaDosElemsPorFace(5,conecLadaisElem(6,nel))
...
!     
      ...
      integer, dimension(numLadosELem,*)   :: conecLadaisElem,listaDosElemsPorFace
...
!          if(nsd==3) then
!          n(1)= listaDosElemsPorFace(3,conecLadaisElem(1,nel))
!          n(2)= listaDosElemsPorFace(4,conecLadaisElem(2,nel))
!          n(3)= listaDosElemsPorFace(1,conecLadaisElem(3,nel))
!          n(4)= listaDosElemsPorFace(2,conecLadaisElem(4,nel))
!          n(5)= listaDosElemsPorFace(6,conecLadaisElem(5,nel))
!          n(6)= listaDosElemsPorFace(5,conecLadaisElem(6,nel))
!          endif
 
!            write(*,'(a,i8,6i8)'), "vizinhosLadal", nel, n(1:6)
!     
         if(n(1)==0) then
            u1=satElem(nel)
         else
...
!=======================================================================
!     
      function udd_rt(dt,ns,hx,hy,hz,uc,uel,unb,vel,flux,p,pc,xk,yk,zk,xkb,VDPEDG,nel) 
!     
...
!
      ...
      contains
         ...
         subroutine vizinhanca2(fluxo,uel,vel,unb,xkb,p,pc,numLadosElem)
!
...
!
      do nel=inicio,fim
!     
         call vizinhanca2(fluxo,uel,vel,unb,xkb,p,pc,numLadosElem)     
! 
...
!=======================================================================
!
      subroutine calc_prod(nedfl,nedg,numnp,numelReserv,nsd,nen,ien,ieedg,vel,sat,xx,t0)
!
!     calcula a velocidade nos nos por elemento
!
      use mPropGeoFisica, only: nelxReserv, nelyReserv, nelzReserv, dimx, dimy, dimz, hy, hz
      use mPropGeoFisica, only: xlo, xlt, nr, prod_out, np_rand_prod,ncont_prod,npprod
      

      implicit none
!
      integer :: nedfl,nedg,numnp,numelReserv,nsd,nen,infile
      integer :: np1,np2,np,l1
      integer, dimension(nen,*)     :: ien
      real(8), dimension(numelReserv)     :: sat
      real(8), dimension(1,*)   :: vel
...
!
      if(nsd==2)write(infile,5566)t0,prod*hy
      if(nsd==3)write(infile,5566)t0,prod*(hy*hz)
!
...
! ======================================================================
!                                               
       subroutine satcor(nsd,numel,mat,c,p,p0,phi,phi0,s,sa,dtT)
...
!  
       ...
       real(8), dimension(*)   :: s,sa
       real(8), dimension(*), intent(in)   :: p,p0,phi,phi0
       integer :: m,nel
       real(8) :: beta,xlambda, xmu
       real(8) :: dtT, dtH
...
!        s(nel)=phi(nel)/phi0(nel)*sa(nel)!*dexp(-beta*(p(nel)-p0(nel)))

!        s(nel)=phi0(nel)/phi(nel)*s(nel) !*dexp(-beta*(p(nel)-p0(nel))) !Antigo
      s(nel)=sa(nel)*(phi0(nel)/phi(nel))**(dtT/dtH) !corrigido
! 
       end do ! nel
! 
       return
...
!**** NEW ******************************************************************* 
!                                               
       SUBROUTINE SATCOR1(NUMELRSRV,phi,phi0,s,sa,dtT)
!
       use mGlobaisEscalares, only: dtBlocoTransp
! 
!      Objetivo: passo corretor para a saturacao
...
!  
       IMPLICIT NONE
       INTEGER :: NEL, NUMELRSRV
       REAL(8) :: dtT, dtH
       REAL(8), DIMENSION(NUMELRSRV)             :: S, SA
       REAL(8), DIMENSION(NUMELRSRV), INTENT(IN) :: PHI, PHI0
!
       dtH=dtBlocoTransp
! 
       DO NEL=1,NUMELRSRV
