...
!  
  ...
  module mSolucoesExternas
      ...
      include 'mpif.h'
        ...
        REAL*8,  allocatable :: initialGuessVel(:), initialGuessGeo(:)
...
#ifdef withOMP
!$OMP PARALLEL
       iparmV(3) =   omp_get_num_threads()
!$OMP END PARALLEL
#endif
       print*, "em pardiso V com, numThreads", iparmV(3) 
...
#ifdef withOMP
!$OMP PARALLEL
       iparmG(3) =   omp_get_num_threads()
!$OMP END PARALLEL
#endif
       print*, "em pardiso G com, numThreads", iparmG(3) 
...
!
         ...
         call montarLmStencilNodal_CRS    (LMstencilEqGeo,listaDosElems, id, &
...
!
         ...
         call montarLmStencilLadal_CRS    (LMstencilEqVel,listaDosElems, id, &
...
! 
      ...
      !**** new *************************************************************
      !
      subroutine montarLmStencilNodal_CRS(LMstencilEq, listaDosElems, id, &
        conectsElem, numCoefPorLinha, ndof, numConexoes, nen, numConexoesPorElem, neq, simetria) 
!  
...
!            write(*,'(a,i8,a,26i8)'), "LmStencil", numEq, "->",LMstencilEq(numEq,1:numCoefPorLinha)
      ...
      end subroutine montarLmStencilNodal_CRS

     !
      !**** new *************************************************************
      !
      subroutine montarLmStencilLadal_CRS(LMstencilEq, listaDosElems, id, &
...
!        write(*,'(a,i5,a,18i6)'), "LmStencil", l, "->",LMstencilEq(l,1:numCoefPorLinha)

      end do
!      
      end subroutine montarLmStencilLadal_CRS
      !
      !----------------------------------------------------------------------
      !
...
!     Create the rhs and solution
      ...
      integer       :: nnz, i, j, k, ierr, printLevel
      integer       :: local_size, Flower, Fupper
      integer       :: cols(50)
      double precision ::  values(50)
     
    Flower    = Clower_+1; Fupper    = Cupper_+1;
    print*, " atribuindo valores da matrix para o HYPRE_IJMatrix ", Flower, Fupper  
    do i = Flower, Fupper
       nnz    =  Ap_(i+1) - Ap_(i) 
...
!        Set some parameters (See Reference Manual for more parameters) 
         ...
         call HYPRE_ParCSRPCGSetPrintLevel (solver_, 1,      ierr)
...
!       Destroy precond and solver
      ...
      call HYPRE_IJVectorGetValues (u_, local_size, rows_, x_, ierr)
      ...
      subroutine escreverResultados (x_, num_iterations_, final_res_norm_, elapsedT_, myId_, print_solution_)
...
!
    ...
    write(*,*) 'em subroutine escreverResultados'
...
!=======================================================================
!    
     ! subroutine solverHYPRE(alhs_, brhs_,                Ap_, Ai_, neq_, nonzerosT_, myid_, num_procs_, mpi_comm_)
...
!
      ...
      integer, intent(in)    :: Ap_(*), Ai_(*)
      integer, intent(in)    :: neq_, nonzerosT_, myid_, num_procs_
      integer (KIND=8)       :: mpi_comm_
!
#ifdef withHYPRE
...
!     call inicializarMPI                 (myid, num_procs, mpi_comm)
      ...
      print*, "----------------criarSistemaAlgHYPRE"
      call criarSistemaAlgHYPRE           (A, parcsr_A, b, par_b, u, par_u, solver,  &
                                             Clower, Cupper, mpi_comm_)

      print*, "----------------atribuirValoresSistemaAlgHYPRE"
