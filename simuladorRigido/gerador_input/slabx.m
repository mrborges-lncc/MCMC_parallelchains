function slabx(Lx,Ly,Lz,nx,ny,nz,q,nome)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hx = Lx/double(nx);
hy = Ly/double(ny);
hz = Lz/double(nz);
...
n = 1*nx*ny*nz+((nx+1)*ny+(ny+1)*nx)*nz+nx*ny;
fprintf(fid,'%10d%10d%10d%10d\n',n-(nx*ny)+1,n,1,1);
%%% face esquerda %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
n1 = nx*ny+1;
n2 = nx*ny+nx;
...
for i=1:nz
    fprintf(fid,'%10d%10d%10.2e\n%30.2e\n%10d%10d\n',...
        n1+((nx+1)*ny+(ny+1)*nx)*(i-1)+(i-1)*nx*ny,2,q,q,ny-1,2*nx+1);
end
%%% face %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf(fid,'%10d\n',0);
fprintf(fid,'%10d%10d%10d%10d%10d\n',1,1,8,8,0);
...
fprintf(fid,'%10d\n',ny*nz);
for i=1:nz
    ni = n1+((nx+1)*ny+(ny+1)*nx)*(i-1)+(i-1)*nx*ny+nx;
    nf = n2+((nx+1)*ny+(ny+1)*nx)*(i-1)+(i-1)*nx*ny+nx;
    np = 2*nx+1;
    for j=ni:np:nf
        fprintf(fid,'%10d\n',j);
    end
