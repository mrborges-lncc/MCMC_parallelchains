function [model,states,initState0,wellSols,W,dt] = runMechanicSchedule(G,...
    rock,overburden,patm,depth,rhoR,grav,vinj,vprod,BHP,well_r,Tf,Tinj, ...
...
%}
    ...
    switch opt.fluid_model
        ...
        case {'oil water'}
            ...
            if printa == 1
                ...
                two2Dplot(p/1e06, [fluid.rhoWS * fluid.bW(p) fluid.rhoOS * fluid.bO(p)],...
                    '$p\ (MPa)$','$\rho_{\alpha}$','water','oil',1);
    ...
    mech = struct('E', rock.E, 'nu', rock.nu, 'el_bc', el_bc, 'load', loadfun);
