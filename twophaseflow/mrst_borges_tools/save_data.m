function np = save_data(sol,G,W,wellsol,printa,nome,exper,t,n,njump,ndt,rep,smon,pmon,dmon)
    if printa == 1 && mod(n-ndt,njump) == 0 && (n > ndt || n == 0)
        nomeprod = [exper '/prod/prod_' nome '_' num2str(rep,'%d') '.dat'];
        nomewcut = [exper '/prod/wcut_' nome '_' num2str(rep,'%d') '.dat'];
        nomepres = [exper '/pres/presinj_' nome '_' num2str(rep,'%d') '.dat'];
        nomesmon = [exper '/conc/sw_' nome '_' num2str(rep,'%d') '.dat'];
        nomepmon = [exper '/pres/pres_' nome '_' num2str(rep,'%d') '.dat'];
        nomedmon = [exper '/disp/disp_' nome '_' num2str(rep,'%d') '.dat'];
        nw  = numel(W);
        winj= []; wbhp = [];
        for i=1:nw
