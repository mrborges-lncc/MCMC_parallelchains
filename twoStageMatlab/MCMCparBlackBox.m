...
freqj = 10;
prt = 1;
precision = [1e-2; 1e-2; 1e-2];
precision_coarse = 1.5*precision;

%% READ REFERENCE DATA %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
file_ref = '~/MCMC_parallelchains/blackbox/exp/saidaref1.dat';
file_ref = [file_ref; '~/MCMC_parallelchains/blackbox/exp/saidaref2.dat'];
file_ref = [file_ref; '~/MCMC_parallelchains/blackbox/exp/saidaref3.dat'];
...
%                 fine_mesh,coarse_mesh,'poro');
            ...
            for k = 1 : num_datatype
                aux = blackbox(dataref{k}(:,1),thetan(:,chain,k));
                aux = [dataref{k}(:,1) aux];
                csamplen{k,chain} = aux;
            end
...
%                 fine_mesh,coarse_mesh,'poro');
            %% Simulation coarse %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            for k = 1 : num_datatype
                aux = blackbox(dataref{k}(:,1),theta(:,chain,k));
                aux = [dataref{k}(:,1) aux];
                csample{k,chain} = aux;
            end
            %% ACCEPTANCE TEST %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%             alpha = prob_accept2stage(dataref,samplen(:,chain),sample(:,chain),...
%                 precision,csamplen(:,chain),csample(:,chain),...
%                 precision_coarse,num_datatype,coarse_post_ratio);
