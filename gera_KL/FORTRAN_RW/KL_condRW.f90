...
MODULE VARIAVEIS
  ...
  REAL(4)  :: FBETA,FVAR,FMEAN,DIMX,DIMY
  ...
  REAL(4), ALLOCATABLE,DIMENSION(:,:) :: PSI,VET,AVET
  REAL(4), ALLOCATABLE,DIMENSION(:)   :: X,Y,AVAL,THETA
...
!          CALL PRINTBIN(PSI,X,Y,NX,NY,NAME)
           ...
           T_FINAL = TARRAY(1)
           TIME = T_FINAL-T_START
           WRITE(*,333)M,TIME,TARRAY(1),TARRAY(2)
        ENDDO
!
333     FORMAT('TIME TO GEN. REAL.',I5,'(s) =',F12.2,/,&
               'USER TIME        (s) =',F12.2,/,&
               'SYSTEM TIME      (s) =',F12.2,/)
